...
echo "$($PSQL "SELECT DISTINCT(course) FROM courses LEFT JOIN majors_courses USING(course_id) LEFT JOIN students USING(major_id) WHERE (first_name='Obie' AND last_name='Hilpert') OR student_id IS NULL ORDER BY course DESC;")"
